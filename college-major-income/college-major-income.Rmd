...
## Python
```{python fig.width = 20, fig.height = 12}
major_processed_dist_salary = major_processed.copy()
# reorder Major_category according to the Median (top -> highest salary)
# Gotcha: you can't use normal [ subsetting, you need something like iloc
# Wrong
# major_processed_dist_salary['Major_category'][-(major_processed_dist_salary['Median']).argsort()]
# Right
major_processed_dist_salary['Major_category'] = (
  major_processed_dist_salary['Major_category']
  .iloc[(-major_processed_dist_salary['Median']).argsort()]
)
# seaborn/matplotlib why are you like this? 
...
plt.xlim(0, major_processed_dist_salary['Median'].max() + 1000)
ax = sns.boxplot(y = 'Major_category', x = 'Median', data = major_processed_dist_salary, orient='h')
ax.set_xticklabels(list('$' + str(int(x)) for x in ax.get_xticks()))
ax.set_ylabel('Major_category',labelpad = 1)
```
