...
```{r major_highest_earning, fig.width=20}
major_processed %>%
...
## Python
```{python warning = FALSE, message = FALSE, fig.width=20}
major_processed_highest_earning = major_processed.copy().query('Sample_size >= 100').head(20)
ax.clear()
# problem: the majors on the x-axis is sorted differently in Python... 
ax = sns.pointplot(
      y = 'Major', 
      x = 'Median',
      hue = 'Major_category',
...
plt.legend(bbox_to_anchor = (0.75, 1.01), loc = 'lower right', borderaxespad = 0.)
# get the lower and upper 25th/75th percentile for error bar
lower_quartile = major_processed_highest_earning.groupby('Major')['Median'].apply(np.percentile, 25).values
upper_quartile = major_processed_highest_earning.groupby('Major')['Median'].apply(np.percentile, 75).values
major_processed_highest_earning.groupby('Major')['Major','Median'].head(20)
lower_quartile
...
plt.errorbar(
  ...
  xerr = quartiles, capsize = 6, elinewidth = 2, linewidth = 2, fmt = 'o', ecolor='lightgrey')
plt.tight_layout()
plt.show()
```
